...
_fnc_food =  {
    ...
    else
    {
        ...
        switch (life_hunger) do {
            ...
            case 10: {
                [ localize "STR_NOTF_EatMSG_3",true,"slow"] call life_fnc_notification_system;
...
_fnc_water = {
    ...
    else
    {
        ...
        switch (life_thirst) do  {
            ...
            case 20: {
                [ localize "STR_NOTF_DrinkMSG_2",true,"fast"] call life_fnc_notification_system;
                if (LIFE_SETTINGS(getNumber,"enable_fatigue") isEqualTo 1) then {player setFatigue 1;};
            };
            case 10: {
                [ localize "STR_NOTF_DrinkMSG_3",true,"slow"] call life_fnc_notification_system;
                if (LIFE_SETTINGS(getNumber,"enable_fatigue") isEqualTo 1) then {player setFatigue 1;};
            };
        };
    };
...
for "_i" from 0 to 1 step 0 do {
    ...
    if (life_carryWeight > life_maxWeight && {!isForcedWalk player} && {!life_god}) then {
        ...
        if (LIFE_SETTINGS(getNumber,"enable_fatigue") isEqualTo 1) then {player setFatigue 1;};
        [ localize "STR_NOTF_MaxWeight",true,"fast"] call life_fnc_notification_system;
    } else {
