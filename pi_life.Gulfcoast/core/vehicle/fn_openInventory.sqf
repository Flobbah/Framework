...
if ((_vehicle getVariable ["trunk_in_use",false])) exitWith {[ localize "STR_MISC_VehInvUse",true,"fast"] call life_fnc_notification_system};
_vehicle setVariable ["trunk_in_use",true,true];
_vehicle setVariable ["trunk_in_use_by",player,true];
if (!createDialog "TrunkMenu") exitWith {[ localize "STR_MISC_DialogError",true,"fast"] call life_fnc_notification_system;}; //Couldn't create the menu?
...
if (_veh_data select 0 isEqualTo -1) exitWith {closeDialog 0; _vehicle setVariable ["trunk_in_use",false,true]; [ localize "STR_MISC_NoStorageVeh",true,"fast"] call life_fnc_notification_system;};
