...
_sel = lbCurSel _list;
if ((_list lbData _sel) isEqualTo "") exitWith {[ localize "STR_NOTF_didNotSelectVehicle",true,"fast"] call life_fnc_notification_system;};
_vehicle = _list lbData _sel;
_vehicle = life_vehicles select parseNumber(_vehicle);

if ((lbCurSel 2702) isEqualTo -1) exitWith {[ localize "STR_NOTF_didNotSelectPlayer",true,"fast"] call life_fnc_notification_system;};
